...
# CandyMonitor Sparkle 自动化发布脚本
# 用法: ./scripts/release.sh <版本号>
# 示例: ./scripts/release.sh 1.0.12
...
DMG_ARM64="$RELEASE_DIR/CandyMonitor_${VERSION}_arm64.dmg"
DMG_X86_64="$RELEASE_DIR/CandyMonitor_${VERSION}_x86_64.dmg"
...
XCODE_SETTINGS=$(xcodebuild -showBuildSettings -project "$PROJECT_DIR/CandyMonitor/CandyMonitor.xcodeproj" -scheme "CandyMonitor" -configuration "Release" 2>/dev/null)
...
REMOTE_VERIFY_DIR="$(mktemp -d "${TMPDIR:-/tmp}/canmirror-release-verify.XXXXXX")"
cleanup() {
...
# 3. 校验 GitHub 资产并生成签名
PUB_DATE=$(date -R)
URL_ARM64="https://github.com/ShawnRn/CandyMonitor/releases/download/v$VERSION/CandyMonitor_${VERSION}_arm64.dmg"
URL_X86_64="https://github.com/ShawnRn/CandyMonitor/releases/download/v$VERSION/CandyMonitor_${VERSION}_x86_64.dmg"

REMOTE_ARM64="$REMOTE_VERIFY_DIR/CandyMonitor_${VERSION}_arm64.dmg"
REMOTE_X86_64="$REMOTE_VERIFY_DIR/CandyMonitor_${VERSION}_x86_64.dmg"
...
echo "请执行:"
echo "gh release create \"v\$VERSION\" \"$DMG_ARM64\" \"$DMG_X86_64\" --title \"CandyMonitor \$VERSION\" --notes \"更新日志说明\""
